...
import { downloadFile, insertIntoGraph } from "./super-utils";
...
class DumpFile {
  constructor(distributionData, data) {
    ...
    this.originalFilename = this.format === "application/gzip" ? ".gz" : ".ttl";
  }

  get downloadUrl() {
    return downloadFileEndpoint(this.baseUrl).replace(":id", this.id);
  }
  ...
  async parseAndDispatch(targetGraph) {
    ...
    for await (const line of reader) {
      ...
      if (buffer.length >= bufferSize && (!line.trim().length || line.trimEnd().endsWith("."))) {
        try {
          const elapsedSeconds = (Date.now() - startTime) / 1000;
          const avgPerSecond = totalTriples / elapsedSeconds;
          const { quads, prefixes: newPrefixes } = await this.parse(buffer.join("\n"), prefixes);
          console.log(
            "processing batch of",
            quads.length,
            "triples,",
    ...
    if (remaining.trim().length) {
      console.log("Buffer is not empty, processing the remaining of the file:\n" + remaining);
  ...
  async parse(data, prefixes) {
    ...
    return new Promise((resolve, reject) => {
      ...
      parser.parse(dataWithExistingPrefix, (error, quad, extractedPrefixes) => {
        if (error) return reject(error);
        if (quad) {
          quads.push(quad);
        } else {
          prefixes = new Map([...prefixes, ...new Map(Object.entries(extractedPrefixes))]);
  ...
  serializeN3Term(rdfTerm) {
    ...
    if (termType === "NamedNode") {
      ...
      else if (language) return `${sparqlEscapeString(value)}@${language}`;
...
export async function getLatestDumpFile(baseUrl) {
  const datasetEndpoint = syncDatasetEndpoint(baseUrl);
  try {
    const urlToCall = `${datasetEndpoint}?filter[subject]=${SYNC_DATASET_SUBJECT}&filter[:has-no:next-version]=yes`;
    ...
    if (dataset.data.length) {
      ...
      const distributionUri = `${baseUrl}/${distributionRelatedLink}`;
      ...
      const selected_distribution =
        distributions.data.find((d) => d.attributes?.format === "application/gzip") ||
        distributions.data.find((d) => d.attributes?.format === "text/turtle") ||
    ...
    console.log(`Unable to retrieve dataset from ${datasetEndpoint}`);
    throw e;
  }
}
