...
export default class DeltaFile {
  constructor(data) {
    ...
    this.name = data.attributes.name || uuid() + ".json";
...
async function loadTimestampFromJob() {
  const queryStr = `
    ...
    SELECT DISTINCT ?deltaTimestamp WHERE {
      ...
      ?dataObject nie:url ${sparqlEscapeUri(TYPE_DELTA_FILES)}.
    }
...
export async function getSortedUnconsumedFiles(since, baseUrl) {
  const filesEndpoint = syncFilesEndpoint(baseUrl);
  try {
    const urlToCall = `${filesEndpoint}?since=${since.toISOString()}`;
      ...
      json.data.map(async (deltaFileMetadata) => {
        ...
        try {
          const fileResponse = await fetcher(
            `${getFileEndpoint(baseUrl).replace(":id", deltaFileMetadata.id)}`,
            {
              headers: {
                Accept: "application/vnd.api+json",
              },
    ...
    return deltaFiles.sort((a, b) => new Date(a.created) - new Date(b.created));
  } catch (e) {
    console.log(`Unable to retrieve unconsumed files from ${filesEndpoint}`);
    throw e;
  }
}
