...
import {
    ...
    SYNC_FILES_ENDPOINT,
    PREFIXES,
    TASK_TYPE, STATUS_SUCCESS,
    GET_FILE_ENDPOINT, TASK_CONSUME,
...
export default class DeltaFile {
    ...
    async load(processDeltaFileCallback = async (deltaFilePath, deltaFileName) => console.log("do not store or delete delta file by default")) {
        try {
            await this.download();
            ...
            console.log(
                `Successfully loaded file ${this.id} stored at ${this.filePath}`
            );
