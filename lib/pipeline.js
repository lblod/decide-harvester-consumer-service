...
export async function run(deltaEntry) {
  ...
  try {
    ...
    if (deltaContainers.length) {
      ...
      const sortedDeltafiles = [];
      for (const baseUrl of SYNC_BASE_URLS) {
        sortedDeltafiles.push(...(await getSortedUnconsumedFiles(latestDeltaTimestamp, baseUrl)));
      }
      sortedDeltafiles.sort((a, b) => new Date(a.created) - new Date(b.created));
