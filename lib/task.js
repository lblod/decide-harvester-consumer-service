...
export async function graphHasData(graph) {
  const result = await query(`ASK { GRAPH ${sparqlEscapeUri(graph)} { ?s ?p ?o } }`, connectionOptions);
...
export async function getInputContainersForTask(task) {
  const queryStr = `
    ...
    PREFIX mu:    <http://mu.semte.ch/vocabularies/core/>
    SELECT DISTINCT ?inputContainer ?resource ?dataObject ?taskType ?uuid
    WHERE {
        ${sparqlEscapeUri(task.task)} tasks:inputContainer ?inputContainer.
        ?inputContainer tasks:hasHarvestingCollection ?collection;
                         tasks:hasResource ?resource.
  ...
  `;
  return parseResult(await query(queryStr, connectionOptions));
}
    ...
    WHERE {
        GRAPH ?g {
            ...
            OPTIONAL { ?subject dct:modified ?modified. }
              ${extMetadataUri ? ` OPTIONAL {?subject ext:metadata ?metadata.}` : ""} 
        }
    }
  `,
    connectionOptions,
...
export async function appendTaskResultContainer(task, container, resourceUri, graphUri = undefined) {
  // prettier-ignore
  const queryStr = `
    PREFIX dct: <http://purl.org/dc/terms/>
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    ...
    INSERT DATA {
       GRAPH ${sparqlEscapeUri(task.graph)} {
        ...
        ${graphUri ? `${sparqlEscapeUri(container.uri)} task:hasGraph ${sparqlEscapeUri(graphUri)}.` : ""}
        ${sparqlEscapeUri(container.uri)} task:hasResource ${sparqlEscapeUri(resourceUri)}.
